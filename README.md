...
- **智能Cookie验证**: 基于时间戳的智能验证机制，减少不必要的API调用
- **配置管理**: 支持自定义数据获取间隔、Cookie配置和验证间隔设置
...
**智能验证机制**：
- 只有在超过设定间隔时间且未进行过其他成功API请求时才执行验证
- 每次成功的数据获取或余额查询会自动更新验证时间戳
- 验证失败时自动停止任务并清理Cookie，用户需重新配置
...
- **智能Cookie验证**：采用基于时间戳的验证策略，减少不必要的验证请求
- **自动失败处理**：Cookie失效时自动停止任务、清理状态，保护系统稳定性
- **成功请求追踪**：每次API请求成功会自动更新验证时间戳，优化验证频次
